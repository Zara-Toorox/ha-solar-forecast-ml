...
class ServiceManager:
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                if self.notification_service:
                    _LOGGER.info("NotificationService Singleton retrieved from hass.data.")
                else:
                    _LOGGER.warning("NotificationService Singleton not found in hass.data. Check __init__.py setup.")
    ...
    async def _initialize_weather_service(self) -> bool:
        """Initialize WeatherService with DataManager for caching."""
        if self.weather_service:
            return True
        if not self._initial_weather_entity:
            _LOGGER.error("Cannot initialize WeatherService: No weather entity configured.")
            return False
        _LOGGER.debug("Initializing WeatherService with cache support...")
        try:
            from ..services.weather import WeatherService
            ...
            weather_init_ok = await self.weather_service.initialize()
            if weather_init_ok:
                _LOGGER.info(
                    f"WeatherService initialized with cache support: "
                    f"{self._initial_weather_entity}"
                )
            else:
                _LOGGER.warning(
                    "WeatherService initialized with warnings "
    ...
    async def _initialize_ml_predictor(self) -> bool:
        ...
        try:
            ...
            # CRITICAL: Configure entities FIRST (especially solar_capacity for peak_power_kw)
            # This ensures peak_power_kw is set before initialize() creates prediction strategies
            ...
            _LOGGER.debug("Entities configured in MLPredictor and SampleCollector (before initialization).")
    ...
    async def cleanup(self) -> None:
        ...
        # Cleanup WeatherService background tasks
        if self.weather_service:
            try:
                await self.weather_service.cleanup()
                _LOGGER.debug("WeatherService cleanup complete")
            except Exception as e:
                _LOGGER.warning(f"Error during WeatherService cleanup: {e}")
