...
âœ… PRODUCTION READY: Async Factory Pattern with Option Validation
âœ… FIX: Centralized Notification Option Checking
âœ… NEW: show_forecast_update, show_training_start, show_training_complete
Version 5.0.0
...
class NotificationService:
    ...
    âœ… ASYNC: All methods non-blocking
    âœ… SAFE: Error handling for all notification operations
    âœ… CENTRAL: Centralized option checking before notifications
    """
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry):
        ...
        âš ï¸ DO NOT CALL DIRECTLY: Use create_notification_service()
        ...
        _LOGGER.debug("ðŸ”§ NotificationService instance created")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            async with self._notification_lock:
                ...
                self._initialized = True
                _LOGGER.info("âœ… NotificationService successfully initialized")
                return True
                
        except Exception as e:
            _LOGGER.error(
                f"âŒ Error during NotificationService initialization: {e}",
                exc_info=True
            )
            return False
    ...
    # ========================================================================
    # ðŸ”§ FIX: New import mechanism without None problems
    # ========================================================================
    async def _safe_create_notification(
        ...
        âœ… FIX: Uses hass.services.async_call instead of direct import
        """
        if not self._initialized:
            _LOGGER.warning(
                f"âš ï¸ NotificationService not initialized - "
                f"Notification '{notification_id}' will not be displayed"
            )
            return False
        
        try:
            # âœ… FIX: Use hass.services.async_call instead of problematic import
            ...
            _LOGGER.debug(f"ðŸ”§ Notification '{notification_id}' created")
            return True
            
        except Exception as e:
            _LOGGER.error(
                f"âŒ Error creating notification '{notification_id}': {e}",
                exc_info=True
            )
            return False
    ...
    async def _safe_dismiss_notification(self, notification_id: str) -> bool:
        ...
        âœ… FIX: Uses hass.services.async_call
        """
        if not self._initialized:
            return False
        
        try:
            # âœ… FIX: Use hass.services.async_call
            await self.hass.services.async_call(
                domain="persistent_notification",
                service="dismiss",
                service_data={
            ...
            _LOGGER.debug(f"ðŸ”§ Notification '{notification_id}' dismissed")
            return True
            
        except Exception as e:
            _LOGGER.warning(
                f"âš ï¸ Error dismissing notification '{notification_id}': {e}"
            )
            return False
    # ========================================================================
    # END FIX
    ...
    async def show_startup_success(
        ...
        âœ… Option Check: Respects CONF_NOTIFY_STARTUP
        ...
        try:
            ...
            if ml_mode:
                # âœ… ML Mode active - all dependencies present
                message = f"""
**ðŸŽ« Solar Forecast ML 6 RISA Started!**

**Status: Full ML Mode âœ…**
...
"""
                ...
                # âš ï¸ Fallback Mode - Dependencies missing
                missing_list = ""
                if missing_packages:
                ...
                message = f"""
**âš ï¸ Solar Forecast ML Started in Fallback Mode**
...
âŒ Automatic model improvement
...
"""
            ...
            _LOGGER.error(f"âŒ Error showing startup notification: {e}", exc_info=True)
            return False
    ...
    async def show_forecast_update(
        ...
        âœ… Option Check: Respects CONF_NOTIFY_FORECAST
        
        Args:
            forecast_energy: Forecasted energy in kWh
...
"""
            ...
            _LOGGER.error(f"âŒ Error showing forecast notification: {e}", exc_info=True)
            return False
    ...
    async def show_training_start(self, sample_count: int) -> bool:
        ...
        âœ… Option Check: Respects CONF_NOTIFY_LEARNING
...
"""
            ...
            _LOGGER.error(f"âŒ Error showing training start notification: {e}", exc_info=True)
            return False
    ...
    async def show_training_complete(
        ...
        âœ… Option Check: Respects CONF_NOTIFY_SUCCESSFUL_LEARNING
        
        Args:
            success: True if training successful
            accuracy: Optional model accuracy score
            sample_count: Optional number of samples used
...
"""
            ...
            # Dismiss training start notification
            ...
            _LOGGER.error(f"âŒ Error showing training complete notification: {e}", exc_info=True)
            return False
...
async def create_notification_service(
    ...
    âœ… ASYNC: Non-blocking initialization
    âœ… SAFE: Returns None on failure instead of raising exceptions
    ...
    try:
        ...
        if await service.initialize():
            _LOGGER.info("âœ… NotificationService created successfully")
            return service
        else:
            _LOGGER.warning("âš ï¸ NotificationService created but not initialized")
            return service  # Return even if not initialized (no persistent_notification)
            
    except Exception as e:
        _LOGGER.error(
            f"âŒ Failed to create NotificationService: {e}",
            exc_info=True
        )
        return None
