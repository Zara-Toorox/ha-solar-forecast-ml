...
# Import only used constants
from .const import (
    CONF_TEMP_SENSOR, CONF_WIND_SENSOR, CONF_RAIN_SENSOR,
    CONF_UV_SENSOR, CONF_LUX_SENSOR
    # CONF_CURRENT_POWER # Removed - Not used directly by collector logic anymore
)
...
class SensorDataCollector:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry):
        ...
        # Map internal keys (used in sensor_data dict) to the config entry keys
        # Maps the keys used internally (e.g., 'temp_sensor') to the keys defined in const.py
        # used in the config flow (e.g., CONF_TEMP_SENSOR which holds 'temp_sensor' string).
        self._sensor_configs = {
            'temperature': CONF_TEMP_SENSOR,
            'wind_speed': CONF_WIND_SENSOR,
            'rain': CONF_RAIN_SENSOR, # Key used internally might differ slightly from const name
            'uv_index': CONF_UV_SENSOR,
            'lux': CONF_LUX_SENSOR,
            # 'current_power': CONF_CURRENT_POWER # Removed - power_entity is handled separately
        }
        _LOGGER.debug("SensorDataCollector initialized.")
    ...
    def get_sensor_entity_id(self, internal_key: str) -> Optional[str]:
        ...
        Args:
            internal_key: The internal key used (e.g., 'temperature', 'wind_speed').
    ...
    def collect_all_sensor_data_dict(self) -> Dict[str, Optional[float]]:
        ...
        Keys are the internal names (e.g., 'temperature', 'wind_speed').
