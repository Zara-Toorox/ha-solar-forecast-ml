    ...
    async def _load_from_solar_forecast_ml(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for date_str in sorted(daily_temps.keys()):
                ...
                # Calculate values
                humidity_val = round(sum(humidity_vals) / len(humidity_vals), 1) if humidity_vals else 0
                wind_val = round(sum(wind_vals) / len(wind_vals), 1) if wind_vals else 0
                wind_max_val = round(max(wind_vals), 1) if wind_vals else 0
                radiation_val = round(sum(radiation_vals) / len(radiation_vals), 1) if radiation_vals else 0
                rain_val = round(sum(rain_vals), 1) if rain_vals else 0
