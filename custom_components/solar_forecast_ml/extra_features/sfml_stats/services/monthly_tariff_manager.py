...
"""Monthly tariff manager for EEG and Energy Sharing support."""
...
class MonthlyTariffManager:
    """Manages monthly tariffs with smart defaults and manual overrides. @zara

    This service handles:
    - Automatic calculation of weighted average prices per month
    - Manual override capability for actual billed prices
    - EEG share estimation from price differences
    - Grid fee scaling based on annual consumption
    - Finalization of months after billing with historical recalculation
    """
    ...
    def _estimate_eeg_share(
        ...
        Formula: EEG% = (Standard - Weighted) / (Standard - EEG) * 100

        Example:
        - Weighted price: 22.5 ct
        - Standard tariff: 26.0 ct
        - EEG tariff: 18.0 ct
        -> EEG share = (26-22.5)/(26-18) = 43.75%
        """
    ...
    async def calculate_weighted_average_price(
        ...
        This is more accurate than simple average because it considers
        WHEN electricity was actually consumed.
        """
        ...
        for hour_key, hour_data in hours.items():
            # hour_key format: "2025-01-15T14" or similar
    ...
    async def get_monthly_data(self, year: int, month: int) -> dict[str, Any]:
        ...
        # Estimate EEG share if we have enough data
        eeg_share = None
        ...
        # Estimate grid fee based on projected annual import
        # Use current month data to project annual consumption
        ...
        if months_elapsed > 0 and monthly_import > 0:
            ...
            projected_annual = 3000  # Default assumption
        ...
        # Determine effective values (override > auto > default)
    ...
    async def set_monthly_override(
        ...
        Args:
            year: Year
            month: Month (1-12)
            overrides: Dict with keys like 'import_price_ct', 'export_price_ct', etc.
                       Set value to None to remove override.
        """
        month_key = self._get_month_key(year, month)
        data = await self._load_data()
        ...
        # Update overrides, removing None values
    ...
    async def finalize_month(
        ...
        """Mark a month as finalized (billed). @zara

        When finalized:
        - Current override values are locked in
        - Historical daily data can be recalculated with correct prices
        """
        month_key = self._get_month_key(year, month)
        data = await self._load_data()
    ...
    async def _recalculate_month_history(
        ...
        This updates the daily energy history with the finalized prices.
        """
        month_key = self._get_month_key(year, month)
        ...
        for day_key, day_data in days.items():
            ...
            # Recalculate costs with finalized prices
