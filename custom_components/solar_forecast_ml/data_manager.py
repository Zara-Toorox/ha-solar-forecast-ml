...
class DataManager(DataManagerIO):
        ...
        try:
            ...
            async with self._file_lock: # Acquire lock for read-modify-write
                ...
                if any(s.get('timestamp') == timestamp for s in samples):
                    _LOGGER.warning(f"Sample für {timestamp} existiert bereits, überspringe Duplicate.")
                    return
    ...
    async def migrate_timestamps_to_local(self) -> bool:
        ...
        try:
            ...
            # 4. Report results
            if all_success:
                _LOGGER.info("Ã¢Å“â€¦ Timestamp migration to local time completed successfully.")
            else:
                _LOGGER.warning("Ã¢Å¡Â Ã¯Â¸Â Timestamp migration partially failed. Some files skipped. Check logs.")
    ...
    def _migrate_timestamp_string(self, timestamp_str: str) -> str:
        ...
        try:
            ...
            if timestamp_str.endswith('+00:00') or timestamp_str.endswith('Z'):
                ...
                _LOGGER.debug(f"Migrated UTC timestamp: {timestamp_str} Ã¢â€ â€™ {migrated}")
                return migrated
            else:
    ...
    async def _migrate_hourly_samples_timestamps(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(f"Ã¢Å“â€¦ hourly_samples.json: {migrated_count} timestamps migrated.")
            return True
            
        except Exception as e:
            _LOGGER.error(f"Ã¢ÂÅ’ Migration of hourly_samples.json failed: {e}", exc_info=True)
            return False
    ...
    async def _migrate_prediction_history_timestamps(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(f"Ã¢Å“â€¦ prediction_history.json: {migrated_count} timestamps migrated.")
            return True
            
        except Exception as e:
            _LOGGER.error(f"Ã¢ÂÅ’ Migration of prediction_history.json failed: {e}", exc_info=True)
            return False
    ...
    async def _migrate_learned_weights_timestamps(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(f"Ã¢Å“â€¦ learned_weights.json: {migrated_count} timestamps migrated.")
            return True
            
        except Exception as e:
            _LOGGER.error(f"Ã¢ÂÅ’ Migration of learned_weights.json failed: {e}", exc_info=True)
            return False
    ...
    async def _migrate_hourly_profile_timestamps(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(f"Ã¢Å“â€¦ hourly_profile.json: {migrated_count} timestamps migrated.")
            return True
            
        except Exception as e:
            _LOGGER.error(f"Ã¢ÂÅ’ Migration of hourly_profile.json failed: {e}", exc_info=True)
            return False
    ...
    async def _migrate_model_state_timestamps(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(f"Ã¢Å“â€¦ model_state.json: {migrated_count} timestamps migrated, flag set.")
            return True
            
        except Exception as e:
            _LOGGER.error(f"Ã¢ÂÅ’ Migration of model_state.json failed: {e}", exc_info=True)
            return False
    ...
    async def cleanup_duplicate_samples(self) -> Dict[str, int]:
        ...
        Behält jeweils den ERSTEN Eintrag, verwirft spätere Duplikate.
        ...
        try:
            ...
            async with self._file_lock:
                ...
                samples = samples_data.get('samples', [])
                if not samples:
                    _LOGGER.info("Keine Samples vorhanden, Cleanup übersprungen.")
            ...
            _LOGGER.info(
                ...
                f"{len(cleaned)} Samples verbleiben (von {original_count} ursprünglich)."
            )
            return result
    ...
    async def _create_samples_backup(self) -> bool:
        ...
        try:
            if not await self._file_exists(self.hourly_samples_file):
                _LOGGER.debug("Keine hourly_samples.json vorhanden, kein Backup nötig.")
                return True
    ...
    def _validate_cleaned_samples(self, samples: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # Prüfe ob alle Samples die required keys haben
            ...
            # Prüfe ob Timestamps unique sind
    ...
    async def set_last_collected_hour(self, hour: int) -> None:
        ...
        try:
            if not isinstance(hour, int) or hour < 0 or hour > 23:
                _LOGGER.error(f"Ungültige Stunde: {hour}")
                return
